    ...
    <section className="border-t border-border bg-background">
      <div className="max-w-lg mx-auto px-4 py-10">
        ...
        >
          ...
          <p className="text-foreground-muted text-[10px] mb-4">
            * Souvenir crafts: max 1 · Family Plan counts as all 3
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {services.map((service, index) => (
            <motion.button
              ...
              className={`bg-background-card border rounded-xl p-3 text-center cursor-pointer transition-all hover:-translate-y-0.5 group ${
                service.category === 'family'
                  ? 'border-amber-500/50 hover:border-amber-500 col-span-3'
            ...
            >
              {service.category === 'souvenir' && (
                <span className="inline-block text-[8px] text-pink-400 font-bold mb-0.5">Souvenir</span>
              )}
              {service.category === 'family' && (
                <span className="inline-block text-[8px] text-amber-500 font-bold mb-0.5">★ Family Plan (=3 experiences)</span>
              )}
